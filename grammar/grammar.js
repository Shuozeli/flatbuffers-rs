...
module.exports = grammar({
  ...
  rules: {
    ...
    source_file: ($) =>
      seq(
        repeat(choice($.include, $.native_include)),
        optional(
          repeat(
            choice(
              $.namespace_decl,
    ...
    native_include: ($) =>
      seq("native_include", field(FIELD_KEY.include_name, $.string_constant), ";"),
    ...
    union_field_decl: ($) =>
      seq(
        repeat(field(FIELD_KEY.documentation, $.documentation)),
        field(FIELD_KEY.union_field_key, $.full_ident),
        optional(seq(":", field(FIELD_KEY.union_field_value, $.type))),
        optional(field(FIELD_KEY.metadata, $.metadata)),
      ),
    ...
    string_constant: ($) =>
      choice(
        ...
        // Single-quoted strings (non-standard but used in some FlatBuffers tooling)
        token(seq("'", /[^']*/, "'")),
      ),
